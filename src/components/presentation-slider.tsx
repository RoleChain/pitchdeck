...
export interface PresentationSliderRef {
  togglePause: () => void
  toggleSpeech: () => void
  isPaused: boolean
  isSpeaking: boolean
...
const PresentationSlider = forwardRef<PresentationSliderRef>((props, ref) => {
  const [api, setApi] = useState<CarouselApi>()
  const [current, setCurrent] = useState(0)
  const [isSpeaking, setIsSpeaking] = useState(false)
  const [voices, setVoices] = useState<SpeechSynthesisVoice[]>([])
...
PresentationSlider.displayName = 'PresentationSlider'
